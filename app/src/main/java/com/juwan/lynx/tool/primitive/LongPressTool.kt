...
class LongPressTool(
    ...
    override val description: String
        get() = "长按当前截图坐标；使用 0-1000 归一化坐标，${CoordinateArgumentParser.screenBoundsDescription()}；不是设备像素"
    ...
    override suspend fun execute(args: Map<String, String>): String {
        ...
        accessibilityService.performLongPress(px, py)
        return "已长按归一化坐标 ($x, $y) → 像素 ($px, $py)"
    }
}
